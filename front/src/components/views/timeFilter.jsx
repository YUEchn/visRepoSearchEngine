...
const TimeFilter = () => {
  ...
  const drawTimeFilterChart = () => {
    ...
    const margin = { top: 1, right: 5, bottom: 14, left: 10 };
    ...
    const x = d3.scaleBand().domain(xData).range([0, gWidth]);

    const y = d3
      .scaleLinear()
      .domain([0, d3.max(yData)])
      .range([0, gHeight]);
    const rectWidth = gWidth / (xData.length * 1.5);
    wrapperG
      .append("g")
      .selectAll("rect")
      .data(xData)
      .join("rect")
      .attr("x", (d) => x(d) + (x.bandwidth() - rectWidth) / 2)
      .attr("y", (d) => gHeight - y(data[d] || 0))
      .attr("width", rectWidth)
      .attr("height", (d) => y(data[d] || 0))
      .attr("value", (d) => data[d] || 0)
      .attr("fill", "#e8d52f");
    // 背景线条
    wrapperG
      .append("g")
    ...
    var brush = d3
      ...
      .on("brush", function (event) {
        ...
        // labelL
        //   .attr("x", s[0])
        //   .text(Math.round(x.invert(s[0])) * bucketSize);
        // labelR
        //   .attr("x", s[1])
        //   .text((Math.round(x.invert(s[1])) - 1) * bucketSize);
        // move brush handles
        handle
          .attr("display", null)
          .attr(
    ...
    // 选择整个范围
     gBrush.call(brush.move, xData.map(x))
  ...
  return (
    <>
      <div id="time-filter-antd" style={{ width: "20%", height: "100%" }}>
        <RangePicker
          picker="month"
          value={value}
          onChange={(val) => {
            let cal =
              val[0].format("YYYY-MM-DD") + "_" + val[1].format("YYYY-MM-DD");
            setDates(cal);
          }}
        />
      </div>
